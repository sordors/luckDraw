		...
		methods: {
			...
			start() {
				if (!this.run) {
					...
					let card_no_rewrad = this.config.card_no_rewrad ? this.config.card_no_rewrad : 0;
					card_no_rewrad = parseInt(card_no_rewrad);
					card_no_rewrad = isNaN(card_no_rewrad) ? 0 : card_no_rewrad;
					...
					let rewardNum = 10 - card_base_rewrad - card_no_rewrad;
